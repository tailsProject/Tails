...
public interface PlaceRepository extends JpaRepository<Place, Long> {
    ...
    // 카테고리 칩이 여러 cat1을 콤마로 묶어 보낼 수 있어 목록 포함 조건으로 받음
    @Query("""
    ...
    List<Place> searchPlaces(@Param("keyword") String keyword,
                              @Param("cat1List") List<String> cat1List,
                              @Param("cat2") String cat2,
                              @Param("region") String region,
                              @Param("regionAlias") String regionAlias,
                              @Param("minLat") Double minLat,
                              @Param("maxLat") Double maxLat,
                              @Param("minLng") Double minLng,
                              @Param("maxLng") Double maxLng);

    // 검색창 자동완성 — 이름에 keyword가 포함된 장소를 이름순으로 최대 8건, 대소문자 구분 없이 매칭
    List<Place> findTop8ByPlaceNameContainingIgnoreCaseOrderByPlaceNameAsc(String keyword);
}
