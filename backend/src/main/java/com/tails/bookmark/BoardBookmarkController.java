...
public class BoardBookmarkController {
    ...
    @GetMapping("/api/bookmarks/boards")
    @Operation(summary = "내 북마크 목록 조회", description = "로그인한 회원이 북마크한 게시글 목록을 조회합니다. 로그인 필요.")
    public ApiResponse<Page<BoardResponse>> getMyBookmarkedBoards(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                                   @PageableDefault(size = 10) Pageable pageable) {
        return ApiResponse.success(boardBookmarkService.getMyBookmarks(userDetails.getMemberId(), pageable));
    }
}
